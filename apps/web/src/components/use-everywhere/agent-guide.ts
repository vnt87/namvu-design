...
// everything it needs to install NamVu Design, expose it as MCP, and
...
export interface AgentGuideOptions {
   ...
   */
  cliHint?: string;
  /** Optional NamVu Design version/channel; surfaced in the header for support tickets. */
  versionHint?: string;
}
...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# NamVu Design — agent setup guide');
  lines.push('');
  lines.push(
    'You are a coding agent (Claude Code, Codex, Cursor, openclaw, hermes, or similar). ' +
      'The user wants NamVu Design wired into your workflow. NamVu Design is a local ' +
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported NamVu Design version: \`${options.versionHint}\``);
  }
  lines.push('');
  ...
  lines.push('   ```');
  lines.push('');
  lines.push('   If it 404s or times out, ask the user to run `pnpm tools-dev` (dev) or open the NamVu Design app (packaged).');
  lines.push('');
  if (installInfo) {
