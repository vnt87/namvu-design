...
type ProjectChatSendMeta = ChatSendMeta & {
  ...
  queueDrain?: boolean;
  /** The NamVu Design Cloud balance gate already ran for this exact send at
...
function buildCreateDesignSystemFromProjectPrompt(input: {
  ...
  return [
    'Create this project as a complete NamVu Design design system workspace.',
    '',
...
function historyWithWorkspaceContext(
  ...
  const block = [
    ...
    'NamVu Design selected or inferred these workspace contexts for this turn. Treat absolute paths as reference context unless the user explicitly asks to edit them.',
...
export function ProjectView({
  ...
  // An NamVu Design Cloud run needs a wallet, and the ONLY client-side veto is
  ...
  const handleSend = useCallback(
    async (
      ...
      // NamVu Design Cloud pre-run balance gate: a definitively insufficient
  ...
  // "Share to NamVu Design" — kicks off the bundled `od-share-to-community`
...
function pluginWorkflowTitle(action: PluginFolderAgentAction): string {
  return action === 'publish' ? 'Publish repo' : 'NamVu Design PR';
}
...
function pluginWorkflowPlannedSteps(action: PluginFolderAgentAction): string[] {
  ...
  return [
    'Ensure the NamVu Design fork exists',
...
const DESIGN_RESULT_DELIVERY_FAILED_DETAIL =
  'The design result was generated, but NamVu Design could not save it to the project.';
