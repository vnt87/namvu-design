...
// attribute the visit to NamVu Design (per-product revenue/traffic attribution).
//
...
const AGENT_AGNOSTIC_DETAIL_FAILURE_UI: Record<string, RunFailureUi> = {
  ...
  // The fix is updating NamVu Design to a build that bundles a compatible
  ...
  if (agentId === 'amr') {
    if (code === 'AMR_AUTH_REQUIRED') {
      return {
        ...
        // "NamVu Design 智能体尚未登录，前往登录即可正常使用" — single CTA, no
