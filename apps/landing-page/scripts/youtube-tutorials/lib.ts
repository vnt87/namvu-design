 ...
 * nexu-io's NamVu Design product.
 */
export async function isAboutOpenDesign(video: VideoInput): Promise<boolean> {
  const system =
    'You decide whether a YouTube video is specifically about the open-source product "NamVu Design" ' +
    ...
    'Figma, or generic "AI agents / AI coding" roundups that do not focus on NamVu Design. ' +
...
export interface CandidateScore {
  ...
  /** How precisely the video is about NamVu Design specifically. */
 ...
 * an actual (at least partial) tutorial that is squarely about NamVu Design.
 ...
 * (is it NamVu Design) plus two 0-5 axes — completeness (is it a real, followable
 * tutorial) and relevance precision (how squarely it's about NamVu Design). The
...
export async function scoreCandidate(video: VideoInput): Promise<CandidateScore> {
  const system =
    'You gate and score a YouTube video for the NamVu Design tutorials catalogue. ' +
    'NamVu Design (github.com/nexu-io/open-design, ~50k stars) is an open-source self-evolving design ' +
    ...
    'focused on NamVu Design. Score two axes 0-5: ' +
    ...
    '"relevance" = how squarely the video is specifically about NamVu Design (5 = dedicated to it; ' +
...
export async function generateCopy(video: VideoInput): Promise<GeneratedCopy> {
  const system =
    'You write catalogue entries for the NamVu Design tutorials page (open-design.ai/tutorials). ' +
    'NamVu Design is an open-source self-evolving design agent by nexu-io. Each entry describes one ' +
