...
summary: 'An archived Gemini CLI walkthrough from an older NamVu Design release, plus the current replacement: configure Google Gemini through BYOK, which runs through the isolated OpenCode adapter.'
date: 2026-05-11
...
This page documents the **Gemini CLI** workflow Sandeep Singh used in [his May 2026 walkthrough](https://www.youtube.com/watch?v=RqjrENimZP4). That direct runtime has since been retired from NamVu Design. The archived sections below explain what appears in the recording; they are not current setup instructions.

To use Google Gemini models in the current release, configure **Google Gemini as a BYOK provider**. NamVu Design executes configured BYOK providers through its isolated OpenCode adapter; it does not detect or invoke the `gemini` executable.
...
2. Open NamVu Design **Settings**, choose the BYOK execution path, select **Google Gemini**, then enter the API key and model.
...
The cost story still depends on the provider plan you bring: NamVu Design is free and open source, while Google controls Gemini API quotas and billing.

![The NamVu Design workspace.](/tutorials/open-design-setup-gemini-cli-free-credits-sandeep-singh/01-workspace.webp)
*The NamVu Design workspace — open-source, local-first, and driven by whichever coding agent you connect.*
...
To use NamVu Design without ever opening the GUI — calling it as a skill or MCP server inside your agent — run:
...
> The steps in this section describe the release shown in the embedded video. They do not work in current NamVu Design releases; use the BYOK setup above instead.

At the time of the recording, this was the step that made the workflow free. NamVu Design drove Gemini CLI and used its account-level allowance.

**Why did the recording use Gemini CLI?** Two reasons, as Sandeep put it: the Gemini models held their own against the rest, and the free tier was generous. Those account-level CLI terms are historical context, not the current NamVu Design integration contract.
...
3. **Point that older NamVu Design release at it.** In the recorded build, restarting NamVu Design made Gemini CLI appear as an available agent for connection testing.
...
NamVu Design keeps your work **project-scoped**: each project lives in its own folder, and switching folders switches projects. Inside a project you can create prototypes, live artifacts, slide decks, images, and even video and audio — not just UI.
...
2. **Write the brief.** Instead of one of the suggested prompts, describe what you want — Sandeep asks for a landing page. You can also attach a screenshot of a layout you like and ask NamVu Design to follow that theme.
...
- **The recording signs in to Gemini CLI with a Google account.** Current NamVu Design releases use a Gemini API key through BYOK instead.
...
- **You do not need a design system to start.** Begin from a built-in one, import a Claude Design project, or let NamVu Design infer defaults.

## FAQ

**Is it really free?**
NamVu Design is free and open source under Apache-2.0. Gemini API usage depends on Google's current free-tier and billing terms; the video's roughly 2% figure came from the retired direct-CLI workflow.

**Which coding agents does it support?**
21+ agents, including Claude Code, Codex, Cursor, GitHub Copilot, OpenCode, and Qwen. NamVu Design detects supported CLIs already installed on your machine and also offers configured BYOK runtimes.

**Do I have to use Gemini CLI?**
No. Current NamVu Design releases do not expose Gemini CLI as a direct runtime. Choose a supported local agent, or configure Google Gemini through BYOK.

**Do I need a design system before I can start?**
No. NamVu Design ships branded design systems and templates as starting points, and you can import an existing design system or let it infer sensible defaults.
