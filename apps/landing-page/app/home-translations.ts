...
export interface HomeExtra {
  ...
  /** Task-first H1 shown below the NamVu Design brand name. */
  ...
  stepDesc4: string;
  capTitle: string;
  labsPre: string; // before <em>NamVu Design</em>
  labsPost: string; // after <em>NamVu Design</em> (incl. punctuation)
  // Lead paragraph under the "What can you make with NamVu Design?" heading.
...
const en: HomeExtra = {
  ...
  heroSub:
    'NamVu Design is the open-source, local vibe design workspace — it turns the coding agents you already run into a design engine that carries you from idea to prototype, web, slides, and HTML video, all finished on your own machine. Agent-native, with 21 coding agents, {systems} design systems, and an Apache-2.0 license.',
  aboutKicker: 'Why NamVu Design?',
  aboutStatement:
    'NamVu Design is the open-source, agentic vibe design workspace — it turns the coding agent you already run into a design engine whose output you fully own. When an LLM first proved it can truly design — real design work, not just copy — that capability arrived closed, hosted, and model-locked. NamVu Design opens it up: local, BYOK, Apache-2.0.',
  ...
  labsPost: '?',
  labsLead:
    'Prototypes, landing pages, slide decks, dashboards, brand and design systems — even HTML video. NamVu Design turns each into real, runnable files, driven by your own coding agent and ready to ship or hand off to engineering.',
  ...
  testiPost: 'are building NamVu Design together',
  newsTitle: 'The NamVu Design newsletter',
  newsDesc:
  ...
  faqTitle: 'NamVu Design FAQ — open source, local-first, and Claude Design alternative',
  footProduct: 'Product',
...
const zh: HomeExtra = {
  ...
  heroSub:
    'NamVu Design 是开源、本地运行的设计工作台——把你已有的 Coding Agent 变成设计引擎，从想法到原型、网页、演示文稿和 HTML 视频，全流程在你自己的设备上完成。\n支持 21 款编程助手、{systems} 套设计系统，采用 Apache-2.0 许可证。',
  aboutKicker: '为什么选择 NamVu Design？',
  aboutStatement:
    'NamVu Design 是开源、本地运行的智能设计工作台——把你已经在用的编程助手变成设计引擎，成果文件完全归你。当大语言模型第一次证明它能真正完成设计，而不只是撰写文案时，这项能力却被封装在闭源、云端托管且绑定模型的产品里。NamVu Design 将它开放出来：本地运行、可自由选择模型，并采用 Apache-2.0 许可证。',
  aboutTab1: '桌面端原生',
  aboutTab2: '不另造助手，接入你已有的工具',
  aboutTab3: '越用越懂你',
  aboutCap1: '设计在桌面端发生。\n本地文件、Figma 导出和代码仓库都能直接读取，编程助手也能完整使用终端能力',
  aboutCap2: '你电脑上的 Claude Code / Codex / Cursor 已经够强。\nNamVu Design 做的是把它们接进完整设计工作流',
  ...
  labsPost: ' 能产出什么？',
  labsLead:
    '原型、落地页、演示文稿、数据看板、品牌与设计系统，甚至 HTML 视频。NamVu Design 会把它们变成真正可运行的文件，由你自己的编程助手驱动，随时可以交付或交接给工程团队。',
  methodTitle: '连接 21+ 款编程助手，开箱即用',
  ctaTitle: '让最前沿的 AI 设计能力回到每一个创作者的桌上',
  testiPre: '来自全球，',
  testiMid: ' 贡献者',
  testiPost: '正在一起构建 NamVu Design',
  newsTitle: 'NamVu Design 订阅',
  newsDesc: '新模板、设计系统更新、大使活动与产品动态，直接发到你的邮箱。',
  newsBtn: '订阅',
  newsDone: '已收到，感谢关注！',
  newsError: '订阅失败，请稍后重试。',
  faqTitle: 'NamVu Design 常见问题 —— 开源、本地优先、Claude Design 替代',
  footProduct: '产品',
  footCommunity: '社区',
  footLegal: '法律',
  footPartners: '伙伴',
...
const zhTw: HomeExtra = {
  ...
  aboutKicker: '為什麼選擇 NamVu Design？',
  aboutStatement:
    '2026 年 4 月，Claude Design 首次證明 LLM 能真正做設計，不是寫文章，而是直接產出設計稿。但它閉源、付費、只跑在雲端，模型鎖 Anthropic，換 Agent、自部署、BYOK 全做不到。NamVu Design 讓這套能力變得開放。',
  aboutTab1: '桌面端原生',
  aboutTab2: '不造 Agent，接入 Agent',
  aboutTab3: '越用越懂你',
  aboutCap1: '設計在桌面端發生。\n本地檔案、Figma 匯出、程式碼倉庫直接可讀，Agent 擁有終端執行全部能力',
  aboutCap2: '你電腦上的 Claude Code / Codex / Cursor 已經夠強。\nNamVu Design 做的是把它們接進完整設計工作流',
  ...
  testiPre: '來自全球，',
  testiMid: ' 貢獻者',
  testiPost: '正在一起打造 NamVu Design',
  newsTitle: 'NamVu Design 訂閱',
...
const ja: HomeExtra = {
  ...
  heroSub:
    'NamVu Design は、オープンソースでローカル動作するデザインワークスペースです。すでに使っているコーディングエージェントを設計エンジンに変え、アイデアからプロトタイプ、Webサイト、スライド、HTML 動画まで、すべて自分のマシン上で完結します。\n21 種類のコーディングエージェントと {systems} 種類のデザインシステムに対応し、Apache-2.0 で公開しています。',
  aboutKicker: 'なぜ NamVu Design なのか？',
  aboutStatement:
    'NamVu Design は、オープンソースでローカル動作するエージェント型のデザインワークスペースです。すでに使っているコーディングエージェントを、成果物が完全に自分のものになる設計エンジンへ変えます。大規模言語モデルが文章だけでなく本物のデザインも作れると証明された一方、その力はクローズドでクラウド依存、特定モデルに固定されていました。NamVu Design はそれを開放します。ローカルで動き、モデルを自由に選べ、Apache-2.0 で公開されています。',
  ...
  labsPost: ' で何が作れる？',
  labsLead:
    'プロトタイプ、ランディングページ、スライド、ダッシュボード、ブランド／デザインシステム、さらに HTML 動画まで。NamVu Design はそれぞれを実際に動くファイルに変換し、自分のコーディングエージェントで制作して、そのまま納品や開発への引き継ぎができます。',
  methodTitle: '21+ のコーディングエージェントを設定ゼロで接続',
  ctaTitle: '最先端の AI デザイン能力を、すべての作り手の手元へ',
  testiPre: '世界中から、',
  testiMid: ' 人のコントリビューター',
  testiPost: 'が一緒に NamVu Design を作っています',
  newsTitle: 'NamVu Design ニュースレター',
  newsDesc:
    '新しいテンプレート、デザインシステムの更新、アンバサダーイベント、プロダクトの最新情報を、あなたの受信箱へ直接お届けします。',
  newsBtn: '購読する',
  newsDone: 'ご登録ありがとうございます！',
  faqTitle: 'NamVu Design のよくある質問 — オープンソース・ローカルファースト・Claude Design 代替',
  footProduct: '製品',
...
const ko: HomeExtra = {
  ...
  heroSub:
    'NamVu Design는 오픈소스이자 로컬로 동작하는 vibe design workspace입니다——이미 사용 중인 코딩 에이전트를 디자인 엔진으로 바꿔, 아이디어에서 프로토타입, 웹, 슬라이드, HTML 비디오까지 전 과정을 내 컴퓨터에서 끝냅니다.\n에이전트 네이티브, 21개 코딩 에이전트, {systems}개 디자인 시스템, Apache-2.0.',
  aboutKicker: '왜 NamVu Design인가?',
  aboutStatement:
    'NamVu Design은 오픈소스이자 agentic한 vibe design workspace입니다. 이미 쓰는 코딩 에이전트를, 결과물이 온전히 내 것이 되는 디자인 엔진으로 바꿉니다. LLM이 처음으로 진짜 디자인을——카피가 아니라 실제 디자인을——해낼 수 있음을 증명했을 때, 그 능력은 폐쇄적이고 호스팅형이며 모델에 묶여 있었습니다. NamVu Design은 그것을 엽니다: 로컬, BYOK, Apache-2.0.',
  aboutTab1: '데스크톱 네이티브',
  ...
  labsLead:
    '프로토타입, 랜딩 페이지, 슬라이드, 대시보드, 브랜드 및 디자인 시스템, 그리고 HTML 비디오까지. NamVu Design은 각각을 실제로 실행되는 파일로 만들며, 내 coding agent로 구동해 바로 배포하거나 엔지니어링에 넘길 수 있습니다.',
  methodTitle: '21개 이상의 코딩 에이전트를 설정 없이 연결',
  ctaTitle: '최첨단 AI 디자인 역량을 모든 창작자의 책상으로',
  testiPre: '전 세계에서, ',
  testiMid: '명의 기여자',
  testiPost: '가 함께 NamVu Design을 만들고 있습니다',
  newsTitle: 'NamVu Design 뉴스레터',
  newsDesc:
    '새 템플릿, 디자인 시스템 업데이트, 앰배서더 이벤트, 제품 소식을 받은편지함으로 바로 보내드립니다.',
  newsBtn: '구독',
  newsDone: '구독해 주셔서 감사합니다!',
  faqTitle: 'NamVu Design FAQ — 오픈소스, 로컬 우선, Claude Design 대안',
  footProduct: '제품',
  footCommunity: '커뮤니티',
  footLegal: '법적 고지',
  footPartners: '파트너',
...
const de: HomeExtra = {
  ...
  heroSub:
    'NamVu Design ist der quelloffene, lokale vibe design workspace — er verwandelt die Coding-Agents, die du bereits nutzt, in eine Design-Engine, die dich von der Idee bis zu Prototyp, Web, Slides und HTML-Video bringt, alles auf deinem eigenen Rechner.\nAgent-native, mit 21 Coding-Agents, {systems} Design-Systemen und Apache-2.0-Lizenz.',
  aboutKicker: 'Warum NamVu Design?',
  aboutStatement:
    'NamVu Design ist der quelloffene, agentische Vibe Design Workspace – er verwandelt den Coding-Agent, den du bereits nutzt, in eine Design-Engine, deren Ergebnisse ganz dir gehören. Als ein LLM erstmals bewies, dass es wirklich gestalten kann – echte Designarbeit, nicht nur Text –, kam diese Fähigkeit geschlossen, gehostet und modellgebunden. NamVu Design öffnet sie: lokal, BYOK, Apache-2.0.',
  ...
  labsPost: ' erstellen?',
  labsLead:
    'Prototypen, Landingpages, Slides, Dashboards, Marken- und Design-Systeme – sogar HTML-Video. NamVu Design macht daraus echte, lauffähige Dateien, angetrieben von deinem eigenen Coding-Agent, bereit zum Ausliefern oder zur Übergabe an die Entwicklung.',
  ...
  testiPost: 'gemeinsam an NamVu Design',
  ...
  faqTitle: 'NamVu Design FAQ – Open Source, lokal und Claude-Design-Alternative',
  footProduct: 'Produkt',
  footCommunity: 'Community',
...
const fr: HomeExtra = {
  ...
  heroSub:
    'NamVu Design est le vibe design workspace open source et local — il transforme les agents de code que vous utilisez déjà en moteur de design, de l’idée au prototype, au web, aux slides et à la vidéo HTML, le tout sur votre propre machine.\nAgent-native, avec 21 agents de code, {systems} design systems et une licence Apache-2.0.',
  aboutKicker: 'Pourquoi NamVu Design ?',
  aboutStatement:
    'NamVu Design est le vibe design workspace open source et agentique — il transforme l’agent de code que vous utilisez déjà en un moteur de design dont vous possédez entièrement le résultat. Quand un LLM a prouvé pour la première fois qu’il pouvait vraiment concevoir — du vrai design, pas juste du texte —, cette capacité est arrivée fermée, hébergée et verrouillée à un modèle. NamVu Design l’ouvre : local, BYOK, Apache-2.0.',
  ...
  labsPost: ' ?',
  labsLead:
    'Prototypes, landing pages, slides, dashboards, systèmes de marque et de design — et même vidéo HTML. NamVu Design transforme chacun en fichiers réels et exécutables, pilotés par votre propre agent de code, prêts à livrer ou à transmettre à l’ingénierie.',
  ...
  testiPost: 'construisent NamVu Design ensemble',
  newsTitle: 'La newsletter NamVu Design',
  newsDesc:
  ...
  faqTitle: 'FAQ NamVu Design — open source, local-first et alternative à Claude Design',
  footProduct: 'Produit',
...
const ru: HomeExtra = {
  ...
  heroSub:
    'NamVu Design — это открытый локальный vibe design workspace: он превращает кодинг-агентов, которыми вы уже пользуетесь, в движок дизайна, ведущий от идеи к прототипу, вебу, слайдам и HTML-видео — всё на вашей машине.\nAgent-native, 21 кодинг-агент, {systems} дизайн-систем, лицензия Apache-2.0.',
  aboutKicker: 'Почему NamVu Design?',
  aboutStatement:
    'NamVu Design — это открытый, агентный vibe design workspace: он превращает кодинг-агента, которым вы уже пользуетесь, в движок дизайна, результат которого полностью принадлежит вам. Когда LLM впервые доказал, что действительно умеет проектировать — настоящий дизайн, а не просто текст, — эта способность пришла закрытой, облачной и привязанной к модели. NamVu Design открывает её: локально, BYOK, Apache-2.0.',
  ...
  labsPost: '?',
  labsLead:
    'Прототипы, лендинги, слайды, дашборды, бренд- и дизайн-системы — и даже HTML-видео. NamVu Design превращает каждый в реальные рабочие файлы на вашем кодинг-агенте, готовые к сдаче или передаче разработке.',
  ...
  testiMid: ' участников',
  testiPost: 'вместе создают NamVu Design',
  newsTitle: 'Рассылка NamVu Design',
  newsDesc:
  ...
  faqTitle: 'FAQ NamVu Design — open source, локальный и альтернатива Claude Design',
  footProduct: 'Продукт',
...
const es: HomeExtra = {
  ...
  heroSub:
    'NamVu Design es el vibe design workspace open source y local: convierte los agentes de código que ya usas en un motor de diseño que te lleva de la idea al prototipo, la web, las slides y el vídeo HTML, todo en tu propia máquina.\nAgent-native, con 21 agentes de código, {systems} design systems y licencia Apache-2.0.',
  aboutKicker: '¿Por qué NamVu Design?',
  aboutStatement:
    'NamVu Design es el vibe design workspace open source y agéntico: convierte el agente de código que ya usas en un motor de diseño cuyo resultado es totalmente tuyo. Cuando un LLM demostró por primera vez que podía diseñar de verdad —diseño real, no solo texto—, esa capacidad llegó cerrada, alojada y atada a un modelo. NamVu Design la abre: local, BYOK, Apache-2.0.',
  ...
  labsPost: '?',
  labsLead:
    'Prototipos, landing pages, slides, dashboards, sistemas de marca y de diseño — incluso vídeo HTML. NamVu Design convierte cada uno en archivos reales y ejecutables, impulsados por tu propio agente de código, listos para entregar o pasar a ingeniería.',
  ...
  testiPost: 'construyen NamVu Design juntos',
  newsTitle: 'La newsletter de NamVu Design',
  newsDesc:
  ...
  faqTitle: 'FAQ de NamVu Design — open source, local-first y alternativa a Claude Design',
...
const ptBr: HomeExtra = {
  ...
  heroSub:
    'NamVu Design é o vibe design workspace open source e local: transforma os agentes de código que você já usa em um motor de design que vai da ideia ao protótipo, web, slides e vídeo HTML, tudo na sua própria máquina.\nAgent-native, com 21 agentes de código, {systems} design systems e licença Apache-2.0.',
  aboutKicker: 'Por que NamVu Design?',
  aboutStatement:
    'O NamVu Design é o vibe design workspace open source e agêntico: transforma o coding agent que você já usa em um motor de design cujo resultado é totalmente seu. Quando um LLM provou pela primeira vez que sabia projetar de verdade — design real, não só texto —, essa capacidade chegou fechada, hospedada e presa a um modelo. O NamVu Design a abre: local, BYOK, Apache-2.0.',
  ...
  labsPost: '?',
  labsLead:
    'Protótipos, landing pages, slides, dashboards, sistemas de marca e de design — até vídeo HTML. O NamVu Design transforma cada um em arquivos reais e executáveis, movidos pelo seu próprio coding agent, prontos para entregar ou repassar à engenharia.',
  ...
  testiMid: ' colaboradores',
  testiPost: 'constroem o NamVu Design juntos',
  newsTitle: 'A newsletter do NamVu Design',
  newsDesc:
  ...
  faqTitle: 'FAQ do NamVu Design — open source, local-first e alternativa ao Claude Design',
  footProduct: 'Produto',
...
const it: HomeExtra = {
  ...
  heroSub:
    'NamVu Design è il vibe design workspace open source e locale: trasforma i coding agent che già usi in un motore di design che ti porta dall’idea al prototipo, al web, alle slide e al video HTML, tutto sulla tua macchina.\nAgent-native, con 21 coding agent, {systems} design system e licenza Apache-2.0.',
  aboutKicker: 'Perché NamVu Design?',
  aboutStatement:
    'NamVu Design è il vibe design workspace open source e agentico: trasforma il coding agent che già usi in un motore di design il cui risultato è interamente tuo. Quando un LLM ha dimostrato per la prima volta di saper progettare davvero — vero design, non solo testo —, quella capacità è arrivata chiusa, ospitata e legata a un modello. NamVu Design la apre: locale, BYOK, Apache-2.0.',
  ...
  labsPost: '?',
  labsLead:
    'Prototipi, landing page, slide, dashboard, sistemi di marca e di design — persino video HTML. NamVu Design trasforma ciascuno in file reali ed eseguibili, guidati dal tuo coding agent, pronti da consegnare o passare all’ingegneria.',
  ...
  testiPost: 'costruiscono NamVu Design insieme',
  newsTitle: 'La newsletter di NamVu Design',
  newsDesc:
  ...
  faqTitle: 'FAQ di NamVu Design — open source, local-first e alternativa a Claude Design',
...
const vi: HomeExtra = {
  ...
  aboutKicker: 'Vì sao chọn NamVu Design?',
  aboutStatement:
    'Tháng 4 năm 2026, Claude Design lần đầu chứng minh rằng LLM có thể thực sự thiết kế — không phải viết chữ, mà tạo ra sản phẩm thiết kế thật. Nhưng nó đóng mã nguồn, trả phí và chỉ chạy trên đám mây, khóa vào mô hình Anthropic — không đổi agent, không self-host, không BYOK. NamVu Design mở khả năng đó ra.',
  ...
  aboutCap2:
    'Claude Code / Codex / Cursor trên máy bạn đã đủ mạnh.\nNamVu Design kết nối chúng vào một quy trình thiết kế hoàn chỉnh.',
  ...
  testiPost: 'đang cùng xây dựng NamVu Design',
  newsTitle: 'Bản tin NamVu Design',
  newsDesc:
...
const pl: HomeExtra = {
  ...
  testiPost: 'wspólnie buduje NamVu Design',
  newsTitle: 'Newsletter NamVu Design',
  newsDesc:
...
const id: HomeExtra = {
  ...
  aboutKicker: 'Mengapa NamVu Design?',
  aboutStatement:
    'Pada April 2026, Claude Design pertama kali membuktikan bahwa LLM benar-benar bisa mendesain — bukan menulis teks, melainkan menghasilkan karya desain nyata. Tapi ia tertutup, berbayar, dan hanya cloud, terkunci pada model Anthropic — tanpa ganti agen, tanpa self-hosting, tanpa BYOK. NamVu Design membuka kemampuan itu.',
  ...
  aboutCap2:
    'Claude Code / Codex / Cursor di mesin Anda sudah cukup kuat.\nNamVu Design menyambungkannya ke alur desain yang lengkap.',
  ...
  testiPost: 'membangun NamVu Design bersama',
  newsTitle: 'Newsletter NamVu Design',
  newsDesc:
...
const nl: HomeExtra = {
  ...
  aboutKicker: 'Waarom NamVu Design?',
  aboutStatement:
    'In april 2026 bewees Claude Design voor het eerst dat een LLM echt kan ontwerpen — geen teksten schrijven, maar echt designwerk maken. Maar het is closed-source, betaald en alleen cloud, vastgezet op Anthropic-modellen — geen agent wisselen, geen self-hosting, geen BYOK. NamVu Design opent die mogelijkheid.',
  aboutTab1: 'Desktop-native',
  ...
  testiMid: ' bijdragers',
  testiPost: 'samen aan NamVu Design',
  newsTitle: 'De NamVu Design-nieuwsbrief',
  newsDesc:
...
const ar: HomeExtra = {
  ...
  aboutKicker: 'لماذا NamVu Design؟',
  aboutStatement:
    'في أبريل 2026، أثبت Claude Design لأول مرة أن نموذج اللغة يمكنه التصميم فعلاً — لا كتابة النصوص، بل إنتاج عمل تصميمي حقيقي. لكنه مغلق المصدر ومدفوع وسحابي فقط، مقيّد بنماذج Anthropic — لا تبديل للوكيل، ولا استضافة ذاتية، ولا BYOK. يفتح NamVu Design هذه القدرة.',
  ...
  testiMid: ' مساهماً',
  testiPost: 'يبنون NamVu Design معاً',
  newsTitle: 'نشرة NamVu Design البريدية',
  newsDesc:
...
const tr: HomeExtra = {
  ...
  heroSub:
    'NamVu Design, açık kaynaklı ve yerel çalışan vibe design workspace’tir — hâlihazırda kullandığın kodlama ajanlarını, fikirden prototipe, web’e, slaytlara ve HTML videoya kadar her şeyi kendi makinende tamamlayan bir tasarım motoruna dönüştürür.\nAgent-native; 21 kodlama ajanı, {systems} tasarım sistemi ve Apache-2.0 lisansı.',
  aboutKicker: 'Neden NamVu Design?',
  aboutStatement:
    'NamVu Design, açık kaynaklı ve agentic bir vibe design workspace’tir: hâlihazırda kullandığın kodlama ajanını, çıktısı tamamen sana ait olan bir tasarım motoruna dönüştürür. Bir LLM ilk kez gerçekten tasarlayabildiğini — metin değil, gerçek tasarım — kanıtladığında, bu yetenek kapalı, barındırılan ve modele kilitli geldi. NamVu Design onu açar: yerel, BYOK, Apache-2.0.',
  ...
  labsLead:
    'Prototipler, açılış sayfaları, slaytlar, dashboardlar, marka ve tasarım sistemleri — hatta HTML video. NamVu Design her birini kendi kodlama ajanınla çalışan gerçek, çalıştırılabilir dosyalara dönüştürür; teslime ya da mühendisliğe devretmeye hazır.',
  ...
  testiMid: ' katkıcı',
  testiPost: 'birlikte NamVu Design’ı inşa ediyor',
  newsTitle: 'NamVu Design bülteni',
  newsDesc:
  ...
  faqTitle: 'NamVu Design SSS — açık kaynak, yerel öncelikli ve Claude Design alternatifi',
  footProduct: 'Ürün',
...
const uk: HomeExtra = {
  ...
  aboutKicker: 'Чому NamVu Design?',
  aboutStatement:
    'У квітні 2026 року Claude Design уперше довів, що LLM може справді проєктувати — не писати тексти, а створювати реальну дизайн-роботу. Але він закритий, платний і лише в хмарі, прив’язаний до моделей Anthropic — без зміни агента, без self-hosting, без BYOK. NamVu Design відкриває цю можливість.',
  ...
  testiMid: ' учасників',
  testiPost: 'разом будують NamVu Design',
  newsTitle: 'Розсилка NamVu Design',
  newsDesc:
