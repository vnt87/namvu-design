...
// sees the web's own "Loading NamVu Design…" shell flash between the splash and
...
function buildCrashReportUrl(ctx: RendererCrashScreenContext): string {
  ...
  const body = [
    "**What happened**",
    "The NamVu Design desktop window crashed several times in a row and showed the recovery screen.",
    "",
...
function buildCrashMailtoUrl(ctx: RendererCrashScreenContext): string {
  const subject = `NamVu Design keeps crashing (renderer ${ctx.reason})`;
  const body = [
    "The NamVu Design desktop app crashed several times in a row on my device.",
    "",
...
<html>
  ...
  <body>
    <div class="panel">
      <p class="title">NamVu Design keeps closing on this device</p>
      ...
      <p class="hint">If this keeps happening, quitting and reinstalling NamVu Design usually resolves it.</p>
    </div>
    <script>
      (function () {
...
const SPLASH_STAGE_LABELS: Record<SplashBootStage, string> = {
  starting: "Starting NamVu Design",
...
export function createSplashWindow(): SplashWindowHandle {
  // NamVu Design ships light-only (the theme setting was removed), so pin the
...
export async function createDesktopRuntime(options: DesktopRuntimeOptions): Promise<DesktopRuntime> {
  ...
  const windowTitle = options.windowTitle ?? "NamVu Design";
  const window = new BrowserWindow({
    ...
    // web's own "Loading NamVu Design…" shell.
    show: false,
    title: windowTitle,
    autoHideMenuBar: true,
    ...MAC_WINDOW_CHROME,
  ...
  const requireMainWindowSender = (event: Electron.IpcMainInvokeEvent): void => {
    if (event.sender !== window.webContents) {
      throw new Error("host IPC is only available to the main NamVu Design window");
    }
  };
  ...
  // reveal the web's own dark "Loading NamVu Design…" shell, and (b) the splash
