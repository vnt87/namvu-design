...
export const PLUGIN_SHARE_ACTION_LABELS: Record<PluginShareAction, string> = {
  ...
  'contribute-open-design': 'Contribute to NamVu Design',
};
...
export function renderPluginSharePrompt({ action, sourcePlugin, stagedPath }: PluginSharePromptInput): string {
  ...
  if (action === 'publish-github') {
    return [
      `Publish the local NamVu Design plugin "${title}" as a new public GitHub repository.`,
      '',
      `The plugin source files have been copied into this project at \`${stagedPath}\`.`,
      'Use the local daemon share endpoint so the publish flow runs through NamVu Design\'s validated GitHub path:',
      '',
      '```bash',
  ...
  return [
    `Open a pull request to add the local NamVu Design plugin "${title}" to the NamVu Design repository.`,
    '',
    `The plugin source files have been copied into this project at \`${stagedPath}\`.`,
    'Use the local daemon share endpoint so the contribution flow runs through NamVu Design\'s validated GitHub path:',
    '',
    '```bash',
...
export function upsertSkillPluginCandidateAssistantMessage(
  ...
  upsertMessage(db, run.conversationId, {
    id: messageId,
    role: 'assistant',
    content: `NamVu Design found reusable skill material that can become a plugin: ${candidate.title}`,
    events: [{
