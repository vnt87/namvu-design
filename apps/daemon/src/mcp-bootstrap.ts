...
export async function ensureMcpDaemonUrl(
  ...
  if (plan.action === "none") {
    ...
    throw new Error(
      `The registered NamVu Design runtime is unavailable and cannot be launched (${plan.reason}).`,
    );
  }
  ...
  throw new Error(
    `NamVu Design was launched headlessly but its daemon did not become ready within ${timeoutMs}ms.`,
  );
}
