...
// and attributes it to the NamVu Design open-source project for shareability.
export function buildDesignSystemSkillsMarkdown(input: {
  ...
  lines.push('');
  lines.push(
    'Generated with **NamVu Design** — the open-source, local-first Claude Design alternative. ' +
...
async function migrateLegacyDesignSystemPackage(
  ...
  const title = normalizeTitle(metadata.title ?? firstHeading(body) ?? id);
  const summary = summarize(body) || 'A reusable NamVu Design design system.';
  ...
  await Promise.all([
    ...
    appKitExists
      ? writeIfMissing(
          ...
          `# ${title} UI Kit\n\nThis package was migrated from an earlier NamVu Design design-system workspace. Use \`index.html\` as the applied interface example and replace it with source-backed modular components when new repository evidence is available.\n`,
        )
...
function generatedDesignSystemFileWrites(
  ...
  const summary = input.summary || 'A user-created NamVu Design design system.';
...
function buildDraftDesignSystemBody(input: UserDesignSystemInput & { title: string }): string {
  ...
  const summary = cleanText(input.summary) || 'A user-authored design system for future NamVu Design projects.';
...
A reusable NamVu Design package for ${input.title}.
...
function renderSkill(input: {
  ...
  return `---
name: ${skillName}
description: Use this skill when generating NamVu Design artifacts that should follow ${input.title}.
user-invocable: true
---
...
function renderOverviewHtml(
  ...
  return renderHtmlDocument(
    title,
    `<main class="overview">
      <p class="eyebrow">NamVu Design system</p>
