...
function codexExecutableGuidance(
  ...
  return ` Configured Codex path failed: ${configuredOverridePath}. NamVu Design also detected a PATH Codex CLI at ${pathResolvedPath}. Update CODEX_BIN or clear the custom path to use the detected binary.`;
}
...
async function prepareOpenCodeConnectionTestCwd(tempDir: string): Promise<void> {
  await fsp.writeFile(
    ...
    'NamVu Design OpenCode connection test.\n',
    'utf8',
  );
  try {
