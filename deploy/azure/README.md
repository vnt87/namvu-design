...
Deploy NamVu Design to Microsoft Azure from the published runtime image — the
...
> [!IMPORTANT]
> **These lanes are for evaluation and demos, not durable data.** NamVu Design
...
Both run as a single instance (NamVu Design uses single-writer SQLite).
