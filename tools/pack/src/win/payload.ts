...
export async function buildWinLauncherPayloadArchive(
  ...
  const writeOverlay = async (input: { includeExecutable: boolean }): Promise<void> => {
    ...
    if (input.includeExecutable) {
      await cp(join(builtApp.unpackedRoot, "NamVu Design.exe"), join(overlayRoot, "payload", "NamVu Design.exe"));
    }
    await writeFile(
...
export async function validateWinLauncherPayloadArchive(input: {
  ...
  try {
    ...
    requirePayloadManifestValue(manifest.entry?.executable, "entry.executable", "payload/NamVu Design.exe");

    await stat(join(extractRoot, archiveRelativePath("payload/NamVu Design.exe")));
