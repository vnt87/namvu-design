...
Local packaging control plane for NamVu Design.
...
Public release bundles keep channel-distinct identities: `NamVu Design.app`, `NamVu Design Beta.app`,
`NamVu Design Prerelease.app`, or `NamVu Design Preview.app`. Local `tools-pack install` adds the developer
...
- Finder/manual launches on macOS may not inherit your shell-managed `PATH`. If packaged NamVu Design cannot detect agent CLIs that work in Terminal, expose those binaries to the GUI login environment or launch the packaged app from a shell session that already sees them.
