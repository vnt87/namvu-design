...
describe("release workflows", () => {
  ...
  it("requires Vela CLI for every beta desktop packaging target", async () => {
    ...
    expect(beta).not.toContain("POSTHOG_KEY:");
    expect(beta).not.toContain("POSTHOG_HOST:");
    expect(beta).not.toContain("POSTHOG_CLI_API_KEY:");
    expect(beta).not.toContain("POSTHOG_CLI_PROJECT_ID:");
    ...
    expect(buildWin).toContain('Test-JsonString $manifest.entry.executable "entry.executable" "payload/NamVu Design.exe"');
