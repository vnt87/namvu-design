...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "NamVu Design-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "NamVu Design-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "NamVu Design"),
    installedExePath: join(namespaceRoot, "runtime", "install", "NamVu Design", "NamVu Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "NamVu Design-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "NamVu Design-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "NamVu Design-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "NamVu Design", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "NamVu Design.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "NamVu Design.exe"))).resolves.toMatchObject({
