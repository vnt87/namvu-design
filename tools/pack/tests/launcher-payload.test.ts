...
function createWinPaths(root: string, namespace: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "NamVu Design-release-beta-win-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "NamVu Design-release-beta-win.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "NamVu Design Beta"),
    installedExePath: join(namespaceRoot, "runtime", "install", "NamVu Design Beta", "NamVu Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "NamVu Design-release-beta-win-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "NamVu Design-release-beta-win-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "NamVu Design-release-beta-win-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "NamVu Design Beta", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "NamVu Design.exe"),
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(join(paths.unpackedRoot, "NamVu Design.exe"), "fake executable\n", "utf8");
  await writeFile(
  ...
  return {
    builtApp: {
      ...
      executablePath: join(paths.unpackedRoot, "NamVu Design.exe"),
      source: "namespace",
...
describe("tools-pack launcher payload archives", () => {
  it("builds channel and namespace scoped payload manifests for both desktop platforms", () => {
    ...
    expect(buildMacLauncherPayloadManifest({
      ...
      appBundleName: "NamVu Design Beta.app",
      channel: "beta",
      entry: {
        cwd: "payload/NamVu Design Beta.app",
        executable: "payload/NamVu Design Beta.app/Contents/MacOS/NamVu Design Beta",
      },
      namespace: "release-beta",
      payloadRoot: "payload",
      platform: "darwin",
    ...
    expect(buildWinLauncherPayloadManifest({
      ...
      entry: {
        cwd: "payload",
        executable: "payload/NamVu Design.exe",
      },
      namespace: "release-beta-win",
      payloadRoot: "payload",
      platform: "win32",
  ...
  it.skipIf(process.platform !== "darwin")("creates a mac payload zip with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.appBundleName).toBe("NamVu Design Beta.app");
      expect(manifest.entry.executable).toBe("payload/NamVu Design Beta.app/Contents/MacOS/NamVu Design Beta");
  ...
  it.skipIf(process.platform !== "win32")("creates a Windows payload 7z with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.entry.executable).toBe("payload/NamVu Design.exe");
      expect(manifest.version).toBe(version);
      await expectPathExists(join(extractRoot, "payload", "NamVu Design.exe"));
